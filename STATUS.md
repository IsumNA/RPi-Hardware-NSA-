...
| Live camera testing | ✅ | GUI *LIVE TESTING* + `live.py` + `pi_camera_check.py`. Pi CSI: picamera2 (usually pre-installed on Pi OS — venv needs `--system-site-packages`, **no sudo apt**) → `rpicam-vid`/`libcamera-vid` CLI pipe → GStreamer libcamerasrc → OpenCV USB/V4L2. `requirements-pi.txt` for pip-only picamera2 when apt is unavailable. Windows: DirectShow webcam probing |
| Run history / model archive | ✅ | `nsa/history.py`: every compile + sweep is auto-snapshotted to `outputs/history/<timestamp>_<tag>/` (trained `model.pt`, `summary.json`, validation panel, scaling chart, ONNX / device artifact, package zip) with a one-line record in `outputs/history/index.jsonl`. GUI *HISTORY* screen (first wizard step + results screen) lists past runs with metrics and lets you Open folder / View panel / **Use for live** (reuse a past model with no recompile) / **Load config** — so you never have to re-run a test to refer back to it |
| Resolution vs TOPS scaling chart | ✅ | `nsa/scaling.py`: after each compile writes `outputs/resolution_tops_scaling.png` — input pixels vs effective TOPS for Pi 5 CPU / Hailo-8 / DeepX (peak TOPS dashed); shown on the GUI results screen and archived in run history |
